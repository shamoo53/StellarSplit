...
export class ArchivingService {
  ...
  async archiveSplit(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      // Authorization check
      if (
        reason === ArchiveReason.MANUALLY_ARCHIVED &&
        split.creatorWalletAddress &&
        split.creatorWalletAddress !== archivedBy
      ) {
      ...
      this.logger.log(
        `Split ${splitId} archived successfully. Reason: ${reason}`,
      );
      return archive;
    });
  }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (archive.archivedAt < ninetyDaysAgo) {
        throw new BadRequestException(
          "Cannot restore archives older than 90 days",
        );
      }
      ...
      if (existing) {
        throw new BadRequestException(
          `Split with ID ${splitData.id} already exists`,
        );
      }
  ...
  async restoreSplitByOriginalId(originalSplitId: string): Promise<Split> {
    ...
    if (!archive) {
      throw new NotFoundException(
        `No archive found for split ID ${originalSplitId}`,
      );
    }
  ...
  async sendReminders() {
    ...
    for (const split of activeSplits) {
      const expiryDate =
        split.expiryDate ||
        new Date(split.createdAt.getTime() + 30 * 24 * 60 * 60 * 1000);
      const timeUntilExpiry = expiryDate.getTime() - now.getTime();
      const daysUntilExpiry = Math.ceil(
        timeUntilExpiry / (1000 * 60 * 60 * 24),
      );
      const daysSinceCreation = Math.floor(
        (now.getTime() - split.createdAt.getTime()) / (1000 * 60 * 60 * 24),
      );
  ...
  private async sendReminder(
    ...
    try {
      ...
      this.logger.error(
        `Failed to send reminder to ${participant.userId}`,
        error,
      );
    }
  }
